...
Current Step: RAGChatService 已自动计算 Context 预算；下一步接入 PromptCenter
```
...
| 5.9 RAG End-to-End & Review | 进行中，已接入预算计算，测试待补 | `system_prompt + query + output limit + safety margin` 自动计算 Context 预算 |
