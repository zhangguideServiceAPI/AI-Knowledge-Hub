...
本文记录 Story 3.6 已实现的文件列表、详情、下载、删除和 Owner-only 权限边界。
当前 Provider 是 LocalStorage；MinIO 和短期 Signed URL 在 Story 3.7 验证。
...
当前 LocalStorage 采用后端代理流：权限检查和对象读取都发生在 FastAPI 内部。`StreamingResponse` 每次读取固定 Chunk，不将整个对象一次载入业务内存，并在完成或异常时关闭文件流。
...
- MinIO Presigned URL、CDN 和公开下载链接。
