...
- 使用 Redis Hash `auth:session:{session_id}` 保存 `user_id`、`refresh_token_hash`、`created_at`、`last_used_at`、`expires_at`、`absolute_expires_at`，以及可选的 `ip_address` 和 `user_agent`。
...
- 普通内部清理可以使用事务 Pipeline 一次提交 `DEL` Session Hash 和 `ZREM` 用户索引成员；客户端 Logout、指定 Session 撤销和全部设备登出使用 Lua，在删除前原子校验 Refresh Hash、目标所有者或当前 Session。
...
- Access Token 使用 `sid` 标记当前 Session。普通业务 API 默认不查询 Redis，敏感 Session 管理接口会二次验证 `sid` 对应的 Session 存在且属于 `sub` 用户。
...
- 事务 Pipeline 避免普通创建或清理时出现只更新一部分 Redis 结构的状态；Lua 进一步保证安全撤销中的校验与删除不可被并发请求分开。
...
- `last_used_at` 在登录和成功 Refresh 时更新，普通 Access API 不为此增加 Redis 写入。
- Refresh Token 原子轮换、Replay Attack、Secret Rotation、IP/User Agent 和当前设备展示均已在 Sprint 2 后续 Story 实现并测试。
