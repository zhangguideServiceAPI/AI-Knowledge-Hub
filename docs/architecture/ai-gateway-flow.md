...
Story 4.2 流程设计已确认。Story 4.3 至 4.8 的 ChatProvider、Factory、Adapter、
AIGateway、ChatService、Router、Prompt Center 与 UsageRepository 已进入可执行链路。
...
Prompt Center 与 UsageRepository 均已接入 ChatService。模型调用期间不持有数据库
事务；成功、失败或取消后才创建短事务记录终态，写入失败不会覆盖模型结果或原错误。
